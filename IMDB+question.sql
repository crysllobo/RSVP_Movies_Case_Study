...
-- Based on the output we can say that the highest number of movies were released in 2017.
-- Also the number of movie releases from 2017 to 2019 has decreased by 1051. 
...
-- Drama genre has a movie count of 4285 being the highest
...
WITH count_genre AS
(SELECT movie_id, COUNT(genre) AS movie_count_genre
FROM genre
GROUP BY movie_id
HAVING movie_count_genre = 1)
...
-- The movies in Action genre has the highest duration of 112.88 seconds followed by Romance and Crime.
...
-- Thriller movies are top 3 and have a movie count of 1484.
...
SELECT * 
FROM prod_house_most
WHERE prod_company_rank =1;

-- Dream Warrior Pictures and National Theatre Live  are the production houses that have produced the most number of hit movies.
...
-- 24 Drama movies where released during March 2017 in the USA having more than 1,000 votes
-- The top 3 genres were Drama, Comedy and Action. 
...
-- There are 8 movies which start with the word 'The' and have an average more than 8
-- The Brighton Miracle has the highest average rating of 9.5 from the Drama genre.
...
-- 361 movies were release between 1st April 2018 and 1st April 2019 have a median rating of 8. 
...
-- Marvel Studios ranks first, Twentieth Century Fox and Warner Bros take the next two top production houses based on the number of votes received by their movies.
...
(SELECT genre, year, title AS movie_name,
		CASE WHEN SUBSTR(worlwide_gross_income, 1, 1) = '$' THEN CAST(TRIM(SUBSTR(worlwide_gross_income,3,LENGTH(worlwide_gross_income)))AS UNSIGNED)
        ELSE ROUND(CAST(TRIM(SUBSTR(worlwide_gross_income,5,LENGTH(worlwide_gross_income)))AS UNSIGNED) / 75) END AS worlwide_gross_income,
		ROW_NUMBER() OVER(Partition BY year ORDER BY CASE WHEN SUBSTR(worlwide_gross_income, 1, 1) = '$' THEN CAST(TRIM(SUBSTR(worlwide_gross_income,3,LENGTH(worlwide_gross_income)))AS UNSIGNED)
					ELSE ROUND(CAST(TRIM(SUBSTR(worlwide_gross_income,5,LENGTH(worlwide_gross_income)))AS UNSIGNED) / 75) END  DESC) AS movie_rank
FROM movie AS m
INNER JOIN genre AS g
ON m.id = g.movie_id
